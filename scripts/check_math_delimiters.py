...
def check_issues(content: str) -> list:
    ...
    # === Check 6: Standalone $$ with no matching partner nearby ===
    for i, m in enumerate(dd_markers):
        classification = classify_dd_position(content, m['pos'])
        if classification == 'STANDALONE':
            # Check if this is part of a valid $$ ... $$ block
            # A valid block formula opener is STANDALONE/LINE_START,
            # closer is STANDALONE/LINE_END
            # If we have two consecutive STANDALONE $$ with content between them,
            # that's fine. But if there's an odd one out...
            pass  # This is complex, skip for now

    # === Check 7: $$ on a line with no other $$ partner (single $$ line in
    # the middle of text, not part of a block) ===
    # Group $$ by line
    dd_by_line = {}
    for m in dd_markers:
        dd_by_line.setdefault(m['line'], []).append(m)

    for line_no, line_markers in dd_by_line.items():
        if len(line_markers) == 1:
            classification = classify_dd_position(content, line_markers[0]['pos'])
            if classification in ('STANDALONE', 'LINE_START'):
                # Check if next $$ is on a later line (could be a block formula)
                pass  # OK for block formulas
            elif classification == 'LINE_END':
                # Check if previous $$ was on an earlier line (closing a block)
                pass  # OK
